...
namespace SwipeK12
{
    ...
    public partial class OpenCard : Window
    {
        ...
        public OpenCard()
        {
            ...
            try
            {
                SwipeDesktop.IdCardUtils.SwipeCard.IDCardsDataTable idCardsTable = scBLL.GetActiveIDCards();

                foreach (DataRow row in idCardsTable.Rows)
                {
                    if (row["SchoolID"].ToString() == Settings.Default.SchoolId.ToString() || row["SchoolID"].ToString() == "-1")
                    {
                        idCardsList.Add(Convert.ToInt32(row[App.APP_DB_FIELD_CARD_ID]),
                            row[App.APP_DB_FIELD_CARD_NAME].ToString());
                    }
                }
